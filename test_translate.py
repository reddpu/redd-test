...
# # Register a Unicode-compatible font if available
# try:
#     from reportlab.pdfbase import pdfdoc
#     pdfdoc.Font2Type1Font.defaultFontMapping['SimSun'] = 'SimSun'
#     pdfmetrics.registerFont(TTFont('SimSun', 'SimSun.ttf'))  # If SimSun.ttf exists locally
...
# def translate_pdf(input_pdf, output_pdf, update_status):
#     doc = fitz.open(input_pdf)
#     c = canvas.Canvas(output_pdf, pagesize=letter)

#     total_pages = len(doc)
#     update_status(f"Processing PDF: 0/{total_pages} pages completed")

#     for page_num in range(len(doc)):
#         update_status(f"Processing page {page_num + 1}/{total_pages}...")
...
#     messagebox.showinfo("Success", f"Translated PDF saved as: {output_pdf}")
...
#     output_pdf = "FR_translated.pdf"
#     threading.Thread(target=translate_pdf, args=(input_pdf, output_pdf, update_status), daemon=True).start()
...
# # Upload Button
# upload_button = tk.Button(root, text="Upload PDF", width=20, height=2, command=select_and_translate)
# upload_button.pack(pady=20)

# # Status Bar (Bottom Label)
